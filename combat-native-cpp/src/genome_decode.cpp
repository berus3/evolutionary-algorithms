...
/*
static Team* buildTeamFromGenome(const int* genome, int teamId) {
    Team* team = new Team(teamId);

    for (int p = 0; p < PLAYERS_PER_TEAM; p++) {
        Player* player = new Player(p);

        // OJO: esto depende de tu implementación.
        // Si Player(p) asigna StatPoints internamente, y luego vos haces setStatPoints(sp),
        // tenés que evitar leak. Pero NO podés borrar algo que Player va a volver a borrar en su destructor.
        //
        // La opción más segura (si podés) es: que Player tenga un método "replaceStatPoints(sp)"
        // que se encargue bien de liberar/reasignar. Acá dejo el patrón conservador:
        StatPoints* oldSp = player->getStatPoints();
        if (oldSp != nullptr) {
            // Si el destructor de Player borra su StatPoints, NO borres acá.
            // Si el destructor NO lo borra, entonces sí hay leak si no lo borrás.
